...
You can also add the ppa repository at https://launchpad.net/~deepfryed/+archive/ppa and install
the latest packages through apt.
...
local MySQL and PostgreSQL server.
  ...
  * You may need to wait for @shanehanna to release the swift ruby bindings
...
with stock database configs.
  ...
  * 5000 "select * from ..." run back to back, fetching all rows and printing them.
  ...
  † swift does typecasting to 5 basic primitive types at the adapter level
    * Integer/Bignum
    * Float/BigDecimal
    * Time
    * NilClass
